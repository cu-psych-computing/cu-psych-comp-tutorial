...
```{r}
# For example, here it might be useful for the column "conf", which refers to each response confidence condition (high or low confidence), to be in factor form.
...
data2$conf <- as.factor(data2$conf)
data2$conf
...
# Remember that one column of a dataframe is a vector. This means that if I call one column using $ indexing, I can THEN [] index into that column like any old vector!
# Here I am accessing some of the cells of my original data frame, and changing their values to NA. This is what it might have looked like if the code to run my original task malfunctioned, and wasn't recording RT for some of the key presses.
data2$rtFA[c(2, 4, 6, 8)] <- NA
# Now let's take a look at what this dataframe looks like now in the console
data2
# The NA placeholder allows R to work around missing data in all sorts of functions. For example:
...
```{r}
# Let's say I want to...
# Calculate the mean hit rate for this subject across all distractor conditions, but only for high confidence responses.
# I need to OPERATIONALIZE this statement into a series of pieces that can be directly translated into R commands

# First, call HIT RATE, but only the ones for HIGH CONFIDENCE
...
# We want to know which values of the column conf are equal to "hi"
data2$conf == "hi"
...
!(data2$conf == "hi") # This should return TRUE for all the times where conf is "lo"
