...
Now, our pseudocode has just introduced a word we haven't previously seen: *IF.* What we need here is a command that will direct R to judge whether some condition is true, and then perform some action ONLY if that condition is true. If-based code is great for promoting reproducibility. When you don't know the exact numerical index of some information in a dataframe (or something else), you can just tell R to check that a certain condition is true, and you'll always get the result you want, no matter what order your rows are in.
