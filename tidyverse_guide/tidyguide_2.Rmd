...
There are two situations where vectorizer functions come in really handy:

* To vectorize functions to run along every element of a list vector (R's default vectorized functions are all intended for atomic vectors)
* To run more complex, multi-line code on every element of a vector

To illustrate the first use case, we'll use `map()` to vectorize an operation that isn't vectorized by default and run it on every element of a list. Here, I'll create a list where each element is a numeric vector, and then use `map()` to compute the mean of every list element.
...
It's important to remember this: **`map()` always returns a list.** Even if each element of the output list contains an element of the same type, and length 1, `map()` is not going to try to guess anything and turn your output into an atomic vector.
