...
Each model object can be inspected with `summary()` if you so choose.
...
We'll call `tidy()` inside of a `map()` call to repeat it over every element of the `models` column, and create a new list-column where every element contains a sub-df with the model coefficients.
