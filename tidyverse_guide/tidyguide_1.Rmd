...
require(tidyverse)
devtools::load_all("~/Repos/cuPsychRTools")
```
...
### Gathering multiple distinct variables at once

Note that `gather()`, by design, will only create _one value variable at a time._ If you have multiple unique variables in a wide df that are all bound by the same indicator conditions, that's a little less straightforward using `tidyr`, I'll admit.

However, I found myself running into this use case enough that I wrote a helper function in [`cuPsychRTools`](https://github.com/cu-psych-r-users/cuPsychRTools) to handle this. If your column names are formatted the right way, `super_gather()` may be able to help you out!
...
`gather()` alone won't get me all the way there:

```{r}
summaries_qs %>%
  gather(key = "extreme", value = "q_score", starts_with("q"))
```

You can see that the key/indicator column, `extreme`, actually contains information for two distinct variables: which questionnaire the score is from, AND which extreme the score is for. If you wanted to get from this intermediate output to the final desired shape, you'd have to first `separate()` your key column to get those two variables apart, and then `spread()` the questionnaire variables back into their own columns.

With some restrictions, `super_gather()` will do this work for you. You have to specify the names of either your key values or your value variables _as they are spelled in your existing column names,_ and the function will attempt to use those to auto-detect the rest of what it needs. I usually supply whichever of `key_names` or `value_names` has fewer elements.

Here's how it looks if you specify the key names:

```{r}
summaries_qs %>%
  cuPsychRTools::super_gather(key = "extreme",
                              key_names = c("min", "max"),
                              name_order = "value_first")
```

And here's how it looks if you specify the value names:

```{r}
summaries_qs %>%
  cuPsychRTools::super_gather(key = "extreme",
                              value_names = c("q_1", "q_2", "q_3"),
                              name_order = "value_first")
```

## From long to wide

To go from long form to wide form, you'll use `tidyr::spread()`.
...
summaries %>%
  gather(key = "statistic", value = "rt", starts_with("rt")) %>%
  # str_replace() is from stringr--check that ish out!
  mutate(statistic = str_replace(statistic, "rt_", "")) %>%
  spread(key = statistic, value = rt)
```

### Spreading multiple distinct variables at once

Similarly to `gather()`, `spread()` will only let you spread one value column at a time. Again, I don't mind this default behavior because it is nice and predictable, but sometimes you just need to spread multiple variables at once! The counterpart to `super_gather()` in our helper package is `super_spread()`, which was written to look as much like `spread()` as possible. The only difference is that you can specify multiple variables for the values (using `tidyselect` helpers if you wish!) as opposed to just being able to specify one value with `spread()`.
...
summaries_qs_long %>%
  cuPsychRTools::super_spread(key = extreme,
                              starts_with("q"),
                              name_order = "value_first")
```

The `name_order` argument specifies the naming convention for the new columns that are created.

# Coming up in part 2

The next walkthrough will get into working with nested dfs, custom-vectorizing functions using `purrr::map()`, using dfs to store multiple model objects, and more. Stay tuned!