...
# But you have to remember to save stuff to vars (into your environment) if you want to manipulate it in any way
```
...
# MVT: I moved functions -- it needs to go later, doesn't fit here.
...
```{r}
toupper(text.var) # You can coerce things to upper or lowercase with the right function
nchar(text.var) # You can also count the number of characters 
...
#Let's look at how it is now, in it's original form
data2$groupStatus

# the function as.factor() is a COERCING function;  these are a family of functions that take data of one type and change them into another type. Like, you could do as.numeric("9") to change it into the number 9.
