...
Character data is R's all-purpose data type for text-based data. But R knows that sometime you might have a column composed of text to label your within-subject task conditions, between-subject groups, etc. **Factor data** is a datatype built on top of character data that gives you special properties that are useful when a column/vector contains _grouping_ information.
...
Use a logical vector of equal length to your vector of interest; all `TRUE` values will be indexed and all `FALSE` values will not be indexed.
...
There are two specific data types that feature heavily when working with dataframes: [factor data](#factor-data) and [missing data](#non-data-data). If you'd like, you can click the links to go back and review those two data types.
...
We've already used a variety of functions, so this is a brief definition:
