...
It's also easy to introduce errors this way, and with a lot of data it would get tedious. Most of the time, you'll be reading data from an external file, or opening up an exiting dataset in R. Once you find the location of your files, what you do next will depend on the file format.
...
# read.csv, just like read.table but with better defaults fo .csv
# read.delim, for when you have a seperator that's not standard

## For getting data that's already in R
...
Sometimes we want to treat categorical variables as factors, but sometimes we want to pretend their numeric (as in a regression, when binary variables can be coded as 0 and 1). Right now, Condition is coded as a binary numeric variable, but we'd like to use it as a factor, and it would be good if the values were actually labelled. However, rather than overwriting the variable, let's make a second one.
...
Once you've created a data clearning script like this one, you'll have a record of all the edits you've made on the raw data, and you can recreate your cleaned data just by running script again. However, it's often easier to save your cleaned data as it's own file (never overwrite the raw data), so when you come back to do analysis you don't have to bother with all the cleaning steps. 
