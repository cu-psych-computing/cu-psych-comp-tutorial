...
The basic syntax of a for loop is similar to that of an if statement.

for (variable in vector) {run some code}

The specification for a for loop contains two things:
- the loop variable that will represent the CURRENT value inside the loop
- the vector that contains all the values that will be looped over

The loop variable can take any name (that makes sense to you). The loop variable is what you'll use as a placeholder in your loop code So, if I want to run code on the values 13, 14, 15 ... 23, I would specify the vector to be `c(13:23)`.
...
For loops can be really great for repeating a function multiple times. However, for loops have some drawbacks:

- They can be slow. For datasets of <1000 observations, this isn't so big of a deal. But if you are dealing with large datasets (lots of subjects, longitudinal observations, imaging data, etc), this will start to annoy you greatly when your code takes several minutes to run something small.
- As soon as you start using a for loop to do more than a couple commands, you run the risk of introducing typos that you didn't expect.
