...
0. [Finding your way around RStudio](R/1_programming/lessonPart0.html)
...
```{r}
!(data$groupStatus == "HMM") # This should return TRUE for all the times where groupStatus is "LMM"
```
...
```{r}
data$adhdCoded <- ifelse(data$adhd >= 4, "adhd", "no adhd")
```
...
Now, you can call your own homemade function just like any function that already comes with R.
...
And again, just like pre-existing R functions, you can assign the outputs of homemade functions to new variables.
