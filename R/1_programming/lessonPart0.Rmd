---
title: "CU Psych Scientific Computing Workshop"
teaching: 9000
exercises: 42
questions:
...
0. Finding your way around RStudio **this document!**
1. [Variables, data types, vectors](lessonPart1.html)
2. [Coding strategy, relational & logical operators](lessonPart2.html)

# Working with RStudio

## Installing R & RStudio

To install R, visit the [CRAN website](https://cran.r-project.org/mirrors.html) to download the appropriate version of R for your operating system (Linux, Mac, or Windows). Run the installer contained within your downloaded file.
...
Let's say you want to load a CSV file containing data on Shakespeare's plays in R. This file is called `shakes.csv`, and it is located in the folder for this R project. In particular, it is saved inside the "1_programming" subfolder, which is located within the "R" subfolder in the main project folder:

![](../../assets/images/filepath.png)
...
```{r generatefilepath}
here("R", "1_programming", "shakes.csv")
```

### Reading the data into R

Then, to examine this file in R, place the file path inside the parentheses in the `read_csv()` function.

```{r readfile, message=FALSE}
# You can do using the file path outputted by the here() function...
read_csv("/Users/emily/Desktop/cu-psych-comp-tutorial-r-prog-dev-emily/R/1_programming/shakes.csv")

# ...or by placing the here() function itself inside the read_csv() function
read_csv(here("R", "1_programming", "shakes.csv"))
```

### Loading the file as an R object

Finally, to load the file into your R environment so you can do more with it later, give it a name (e.g., `shakespeare`) and use the `<-` (the _assignment operator_) to load the data contained in the CSV into an object with that name.

```{r loadfile, message=FALSE}
shakespeare <- read_csv(here("R", "1_programming", "shakes.csv"))
```
