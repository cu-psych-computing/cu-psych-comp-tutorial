...
# Scale creates a matrix with width 1, sometimes this messes things up so we'll convert it to numeric          
mydata$NeuroticismZ <- as.numeric(mydata$NeuroticismZ)
...
mydata <- mydata %>% mutate(Mean = rowMeans(cbind(Day1, Day2, Day3, Day4)))
...
Sometimes, data might be spread across multiple spreadsheets, and you'll want to combine those for your analysis. For example, maybe this study had a follow-up survey on Day 30. Scores from that survey were entered into another spreadsheet, which only has the subject ID and that score. We want to include that score into our data. Here I'm using the function `merge()`, which is one of the base functions in R, not part of the tidyverse.

```{r}

#First load the new data
mydata2 <- read.table("Study1_Followup.csv", header=TRUE, sep=",", stringsAsFactors=FALSE)

# Merge the two dataframes. To make sure the rows match up, we use the 'by' argument to speficy that IDs should match. That way even if the data is in a different order you will get scores matched togther correctly.

mydata <- merge(mydata, mydata2, by="ID")
...
mydata_Long <- gather(mydata, key="Time", value="Score", starts_with('Day'))
