...
# 1. simple wide-format dataframe, which only includes one conditon of the task
...
df.grouponly <- subset(df, groupStatus != "")
nrow(df.grouponly)
...
leveneTest(df.grouponly$adhd, df.grouponly$groupStatus) 
...
t.test(df.grouponly$adhd ~ df.grouponly$groupStatus)
...
t.test(df.grouponly$adhd ~ df.grouponly$groupStatus, var.equal=TRUE)
```
...
t.test(df_rm$dprime ~ df_rm$distPresent, paired = TRUE)
...
```{r}
aov_1 <- aov(adhd ~ groupStatus, data = df.grouponly)
summary(aov_1) # anova output
coefficients(aov_1) # coefficients (betas) indicating effect for each group.
```
...
aov_2 <- aov(adhd ~ groupStatus:Gender + groupStatus + Gender, data =df.grouponly)
summary(aov_2)
...
aov_3 <-  aov(adhd ~ groupStatus*Gender, data =df.grouponly)
summary(aov_3) # the results are identical to above. 

# the "effect" package allows you to quickly view the fits of your model
plot(allEffects(aov_3))
...
post_hoc <- TukeyHSD(aov_3)
post_hoc # view the output.
...
coplot( adhd ~ mmi|Gender, data = df) 

# we can add a few more arguments to make it pretty
coplot(adhd ~ mmi | Gender, data =df ,number = 3,
...
model4 <- lm(adhd ~ mmi * Gender, data =df)
summary(model4) 
...
```{r}
plot(effect("mmi:Gender", model4), rug = F, main = "", ylab= "ADHD symptoms", xlab = "Media Multitasking")
...
#let's run the model
model5 <- lm(adhd ~ mmi * bis, data =df)
summary(model5)
