...
# Paul's path: setwd("~/Documents/Columbia/Courses/Gelman_Communicating/Project")
# Monica's path:
...
# MT: Style suggestion: always newline after the + in ggplot calls? Makes it a little more readable, and also demonstrates that every new chained command is a LAYER
...
```{r}
# MT: changed this from using car::recode() to plyr::mapvalues() to recode Group, in keeping within tidyverse
require(plyr)
...
# MT: Reiterating previous comment about explaining the args here? if you think that's useful
...
MT: Changing the arg to specify color in the aes() specs of the ggplot() call to be `color` and not `col` for readability (`col` could be columns if you're not used to it)
