...
```{r}
# You can write out un-evaluated conditions inside the if () parentheses too.
if (2 + 2 == 4) {
  cat("We can do math!")
}
```
...
Okay, looks good here! But let's make sure the code does what it's supposed to when the proportion of lefties is actually outside the bounds. We can test it with another simulated dataset (where the generating parameters are different...).
