...
This is where logical & relational operators come in. These are ways that you can specify conditions under which particular operations should be done. In the prior example, this is how we would tell R "Calculate the mean of the "score" column of my dataframe, but only include scores for subjects between the ages of 18 and 22." Let's break this down into its components:

These are the things you need to tell R:
- find which rows of your dataframe belong to subjects aged 18-22
- get those rows out of the column "score" of your dataframe
- calculate a mean using those values

We know how to do the second thing (vector indexing to retrieve specific values!) and the third thing (use the function that does what we want!), but what about the first thing? As above, you could physically look and record which rows belong to subjects aged 18-22, but again, this is time-consuming and sub-optimal. How about we ask R to do the work for us? All of the values R needs are there.

We need to figure out how to make R understand the phrase "subjects aged between 18 and 22." How can we translate this human phrase into R?
