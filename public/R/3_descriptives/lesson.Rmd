...
df <- read.csv("uncapher_2016_repeated_measures_dataset.csv", header=TRUE, stringsAsFactors = FALSE)
...
```{r}
df %>% 
  dplyr::summarise(min_HR = min(hitRate, na.rm = T),
...
write.csv(basic_descriptives, file = "R/3_descriptives/myDescriptives.csv")
save(basic_descriptives, df, file="R/3_descriptives/StudyData.rda")
