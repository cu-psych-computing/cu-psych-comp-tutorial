...
subtitle: "Nested dfs, `map`-ing, and more"
author: "Monica Thieu"
tags: ["extra", "R"]
weight: 702
---
...
Hello (again)! This is a continuation of our tour through base `tidyverse` ([see Part 1 here](/tutorials/r-extra/tidyverse-guide/tidyguide-1/)). Our goals are the same as before:
...
Again, please remember that for more exhaustive documentation you can visit the online reference pages for your package of interest, and for more in-depth self-teaching, please refer to the [R for Data Science online textbook](http://r4ds.had.co.nz/){target="_blank"} by Garrett Grolemund and Hadley Wickham.

# Links to Files

The files for all tutorials can be downloaded from [the Columbia Psychology Scientific Computing GitHub page](https://github.com/cu-psych-computing/cu-psych-comp-tutorial){target="_blank"} using [these instructions](/accessing-files/){target="_blank"}. This particular file is located here: `/content/tutorials/r-extra/tidyverse-guide/tidyguide-2.rmd`.
...
* reshape your df using `pivot_longer()` and `pivot_wider()`
...
1. We folded all the observations into a separate column called `characters` using `nest()`,
1. We used a negative vector to hold grouping-level variables out of that folded column
...
To read more in-depth about vectorized functions in R, check out this handy [blog post](http://www.noamross.net/blog/2014/4/16/vectorization-in-r--why.html){target="_blank"} from Noam Ross. We won't be getting deep into the nuts and bolts of how vectorization works in R here. We'll just be discussing it at a high level that should orient you enough to be able to make quick use of vectorizing helper functions.
...
nested <- raw %>%
  ...
  nest(trials = -c(id, age, gender, q_1, q_2, q_3))
```
  ...
  mutate(summaries = map(trials,
                         ~.x %>%
                           ...
                           mutate(is_old = recode(is_old,
                                                  ...
                                                  `1` = "rate_hit")) %>%
                           # now we will spread
                           pivot_wider(names_from = is_old, values_from = rate_choose_old)))
```
...
* If you really need a progress bar to print to console, for iterating over _looooong_ vectors
    ...
    + Check out the [`progress`](https://github.com/r-lib/progress){target="_blank"} package, which contains helper functions that work inside for loops to make your own progress bars
...
Note that when you unnest, any columns of list type that _aren't_ explicitly unnested will be _repeated_ in your output. We only want to keep the `trials` column, so we use `select()` to drop the `models` list-column from the data before unnesting.
