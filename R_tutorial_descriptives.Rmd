...
# Let's explore a sample dataset: mtcars, available in base R
The data was extracted from the 1974 Motor Trend US magazine, and comprises fuel consumption and 10 aspects of automobile design and performance for 32 automobiles (1973–74 models).
For more information, click here: https://stat.ethz.ch/R-manual/R-devel/library/datasets/html/mtcars.html
...
```{r}
# plot histogram of a continuous variable, miles per gallon: 
hist(mtcars$mpg)

# Next, make your histogram prettier with some extra arguments
# base R graphics have many more options, these are just a few. 
...
# 2.BOXPLOTS 
We will use the basic boxplot function available in base R. 
Boxplots can be created for individual variables quite easily. 
Let's make a boxplot for HP (horsepower).
...
We can also use grouping variables in boxplots. Let's look at horsepower by transmission type 
```{r}
...
# 5. Correlations between two continuous variables
...
# a taste of statistical inference! 
cor.test(mtcars$mpg,mtcars$wt)
```

# 6. Correlations of many variables
...
# this works best when you have 3-5 variables (it gets too hard to read after that)
# so let's look at the first 4 variables of mtcars dataset
pairs (mtcars[1:4])

#We can see some nice patterns here right away, mainly that cyl, disp and hp all appear to be correlated with mpg.
